...
namespace BeepBoopBot.Preconditions
{
    ...
    public class MinPermissionsAttribute : PreconditionAttribute
    {
        ...
        {
            AccessLevel access = GetPermission(context);            // Get the acccesslevel for this context
        ...
        public AccessLevel GetPermission(ICommandContext c)
        {
            ...
            if (c.User.Id == Configuration.Load().BotMaster)     // Give the master even more special access.
                return AccessLevel.BotMaster;

            SocketGuildUser user = c.User as SocketGuildUser;                // Check if the context is in a guild.
            if (user != null)
            {
